...
from kmeans import analyze_clusters_over_time, cluster_pca_results
...
def calculate_trustworthiness_comparison(df_returns_filtered, df_pca, results_tsne, window_size):
    ...
    last_date = last_tsne_df.index[-1] if isinstance(last_tsne_df.index, pd.DatetimeIndex) else df_returns_filtered.index[-1]
        
    end_idx = df_returns_filtered.index.get_loc(last_date) if last_date in df_returns_filtered.index else len(df_returns_filtered)-1
    start_idx = max(0, end_idx - window_size)
        
    X_high = df_returns_filtered.iloc[start_idx:end_idx].T.values 
    X_low_tsne = last_tsne_df[['x', 'y']].values

    common_indices = [idx for idx in last_tsne_df.index if idx in df_pca.index]
    X_low_pca = df_pca.loc[common_indices, ['PC1', 'PC2']].values
        
    X_high = df_returns_filtered.loc[:, common_indices].iloc[start_idx:end_idx].T.values
...
def calculate_comparison_metrics(df_returns_filtered, results_pca, times_pca, results_tsne, window_size):
    ...
    if last_date not in df_returns_filtered.index:
...
if __name__ == "__main__":
    ...
    X_filtered, df_returns_filtered = preprocess_data(df_prices, filter_outliers=True)
    html_parts.extend(build_header(X_filtered))
    ...
    html_parts.extend(build_pca(df_pca, pca_table))
    html_parts.append(build_static_tsne_only(df_tsne_coords))
    ...
    html_parts.extend(build_kmeans(results_pca, times_pca, df_pca, pca_table, N_CLUSTERS))
    ...
    html_parts.extend(build_tsne_evolution(results_tsne, times_tsne, scores_tsne))

    trust_html = calculate_trustworthiness_comparison(df_returns_filtered, df_pca, results_tsne, WINDOW_SIZE)
    if trust_html:
